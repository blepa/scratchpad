	...
	DECLARE @filter_type VARCHAR(10) = 'session'
	DECLARE @not_filter sysname = ''
	DECLARE @not_filter_type VARCHAR(10) = 'session'

	--Retrieve data about the calling session?
	DECLARE @show_own_spid BIT = 0

	--Retrieve data about system sessions?
	DECLARE @show_system_spids BIT = 0

	--Controls how sleeping SPIDs are handled based on the idea of levels of interest
	--0 does not pull any sleeping SPIDs
	--1 pulls only those sleeping SPIDs that also have an open transaction
	--2 pulls all sleeping SPIDs
	DECLARE @show_sleeping_spids TINYINT = 1

	--If 1 gets the full stored procedure or running batch when available
	--If 0 gets only the actual statement that is currently running in the batch or procedure
	DECLARE @get_full_inner_text BIT = 0

	--Get associated query plans for running tasks if available
	--If DECLARE @get_plans = 1 gets the plan based on the request's statement offset
	--If DECLARE @get_plans = 2 gets the entire plan based on the request's plan_handle
	DECLARE @get_plans TINYINT = 0

	--Get the associated outer ad hoc query or stored procedure call if available
	DECLARE @get_outer_command BIT = 0

	--Enables pulling transaction log write info and transaction duration
	DECLARE @get_transaction_info BIT = 0
	...
	DECLARE @get_task_info TINYINT = 1

	--Gets associated locks for each request aggregated in an XML format
	DECLARE @get_locks BIT = 0

	--Get average time for past runs of an active query
	--(based on the combination of plan handle sql handle and offset)
	DECLARE @get_avg_time BIT = 0
	...
	--
	--If DECLARE @get_task_info is set to 2 and a lock wait is detected a subnode called block_info will be
	--populated with some or all of the following: lock_type database_name object_id file_id hobt_id 
	--applock_hash metadata_resource metadata_class_id object_name schema_name
	DECLARE @get_additional_info BIT = 0

	--Walk the blocking chain and count the number of 
	--total SPIDs blocked all the way down by a given session
	--Also enables task_info Level 1 if DECLARE @get_task_info is set to 0
	DECLARE @find_block_leaders BIT = 0

	--Pull deltas on various metrics
	--Interval in seconds to wait before doing the second data pull
	DECLARE @delta_interval TINYINT = 0
	...
	DECLARE @output_column_list VARCHAR(8000) = '[dd%][session_id][sql_text][sql_command][login_name][wait_info][tasks][tran_log%][cpu%][temp%][block%][reads%][writes%][context%][physical%][query_plan][locks][%]'
	...
	DECLARE @sort_order VARCHAR(500) = '[start_time] ASC'

	--Formats some of the output columns in a more "human readable" form
	--0 disables outfput format
	--1 formats the output for variable-width fonts
	--2 formats the output for fixed-width fonts
	DECLARE @format_output TINYINT = 1
	...
	--a CREATE TABLE statement will be returned via the DECLARE @schema parameter which will match 
	--the schema of the result set that would be returned by using the same collection of the
	--rest of the parameters. The CREATE TABLE statement will have a placeholder token of 
	--<table_name> in place of an actual table name.
	DECLARE @return_schema BIT = 0
